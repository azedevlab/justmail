...
Milestone gates are strict: nothing from M(n+1) starts before M(n) ships to devlab.az.

## Milestone 1 — Core platform (the current target)

**Definition of done:** a fresh Ubuntu server + one deploy command = working mail
hosting for devlab.az, fully managed from the UI; mail received from the internet and
...
- Deploy to devlab.az at least at every step boundary — the server is the demo.
