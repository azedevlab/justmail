...
function Step({ n, children }: { n: number; children: ReactNode }) {
  return (
    <li className="flex items-start gap-3">
      <span className="w-5 h-5 shrink-0 grid place-items-center rounded-full bg-[var(--color-accent-subtle)] text-[var(--color-accent)] text-[11px] font-semibold tabular-nums mt-px">
        {n}
      </span>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {PLANNED.map((p) => (
            <Card key={p.title} className="p-5">
              <span className="w-9 h-9 grid place-items-center rounded-lg bg-[var(--color-accent-subtle)] text-[var(--color-accent)] mb-3">
                {p.icon}
              </span>
